...
def main() -> None:
  ...
  parser.add_argument(
    "--pairs",
    default=None,
    help="比較ペア A,B;...（A-B のマージンを集計。省略時は human,MID;MID,copy;human,copy）",
  )
  parser.add_argument(
    "--mid",
    default="fable",
    help="copy と human の間に置く候補の id（v2b=fable, v2c=machine）",
  )
  parser.add_argument("--out", help="マージン分析 JSON の出力先（省略時は stdout 要約のみ）")
  args = parser.parse_args()

  mid = args.mid
  pairs_arg = args.pairs or f"human,{mid};{mid},copy;human,copy"
  ...
  # 項目ごとのスコアと正規化位置（copy=0, human=1 のとき中間候補はどこか）
  # human≈copy のときは分母が潰れるので位置は出さない。
  positions = []
  per_item = []
  for it in items:
    s = it["scores"]
    if not all(k in s for k in ("human", mid, "copy")):
      continue
    span = s["human"] - s["copy"]
    if span > 1e-3:
      pos = (s[mid] - s["copy"]) / span
    else:
      pos = float("nan")
    ...
    per_item.append(
      {
        "id": it["id"],
        "scores": {k: s[k] for k in ("human", mid, "copy")},
        f"margin_human_{mid}": s["human"] - s[mid],
        f"margin_{mid}_copy": s[mid] - s["copy"],
        "margin_human_copy": s["human"] - s["copy"],
        f"{mid}_position": pos,
        "model_best": it.get("best_model"),
        "order": sorted(("human", mid, "copy"), key=lambda k: -s[k]),
      }
    )
  ...
  summary = {
    ...
    f"{mid}_position": {
      ...
      "note": f"0=copy, 1=human。0〜1 なら human>{mid}>copy の間にいる",
    },
    "items": per_item,
  }
  ...
  fp = summary[f"{mid}_position"]
  print(
    f"  {mid}_position (0=copy,1=human): "
