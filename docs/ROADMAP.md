...
| 3j | 文列 Transformer（pref-sentseq） | 3a | GPU（DOK） | **有望・改良中**（2026-07-26）。文単位の凍結 ruri ベクトル列＋段落開始埋め込みを2層 Transformer に読ませる。機械負例なしで v1 top-1 0.80→0.85（bt 0.45）、機械案の相対位置が初めて「コピーと人間の間」に収まった。40 エポック再学習で valid 0.846→0.857・複数段落 0.929 だが train 正解率 0.82 で頭打ち＝容量か学習率の制約。bt との固定重み合成はどの重みでも両課題の最良にならず、当面は二軸運用（ランキング＝sentseq、細部悪化ゲート＝bt）。次は容量増（D_MODEL=512, NUM_LAYERS=4）。LOPO 全 fold は未実施。詳細 [DOK-PREF-SENTSEQ.md](DOK-PREF-SENTSEQ.md) |
