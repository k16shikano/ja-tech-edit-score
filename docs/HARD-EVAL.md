...
```json
{
  ...
  "candidates": [
    { "id": "norms", "text": "…", "generator": "composer", "prompt_tag": "norms" },
    { "id": "plain", "text": "…", "generator": "composer", "prompt_tag": "plain" },
    { "id": "base", "text": "（base_text と同じか、明示的に含める）", "generator": "copy", "prompt_tag": "identity" }
  ],
  "human": {
    "best_id": "norms",
    "rank": ["norms", "plain", "base"],
    "notes": ""
  },
  "status": "labeled"
}
...
規範ありは既存の `japanese-tech-writing` を前置し、末尾は次だけにする。

```
次の下書きを、上記の規範に沿って意味を保ったまま推敲せよ。
出力は推敲後の本文のみ。
...
規範なしは規範ブロックを外し、「意味を保ったまま読みやすく推敲せよ。本文のみ。」とする。
...
3. 人手で `best_id`（できれば `rank`）を付け `labeled` にする
