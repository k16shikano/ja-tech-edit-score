...
- **トークン上限**: CE は `(base, candidate)` を `max_length=512` で切る。
  v2 は tokenizer で全候補のトークン長を実測し、溢れる項目は落とす
  （`make hard-eval-v2-build MAX_TOKENS=512`）。
  節全体が長い場合は、段落数が一致する節から連続 3 段落ウィンドウを切り出す。
  以前の長文セットでは 24 件中 18 件が overflow し、`deg-reverse` が先頭に見える
  アーティファクトで不当に高得点になっていた。
