...
検証セット 651 ペアで人間編集が下書きから稼ぐマージン
（margin = s(source, 人間編集) − s(source, 下書き)）の分布を測った
（`make calibrate-margins` → `outputs/acceptance_margin_calibration.json`）。

| パーセンタイル | pref-sentseq-3e4 | pref-bt |
|---|---:|---:|
| p5 | −0.31 | 0.45 |
| p25 | 0.09 | 5.04 |
| p50（中央値） | 1.89 | 18.59 |
...
- **合格ライン（主軸）**: 候補の sentseq マージン ≥ 中央値 1.9 なら
  「人間編集の中央的な改善幅と同等」。緩めるなら p25 の 0.1（=正であること）。
  `make rank MIN_MARGIN=1.9`。
- **ゲート（bt）**: 既定の 0（悪化していない）で足りる。厳しくするなら p25 の 5.0。
  `GATE_MIN_MARGIN=5`。
