...
| `D_MODEL` | 未設定（256） | 未設定 | 未設定（容量実験では `512`） |
| `NUM_LAYERS` | 未設定（2） | 未設定 | 未設定（容量実験では `4`） |
...
2. **容量**（次の実験: `D_MODEL=512 NUM_LAYERS=4 EPOCHS=40`）: Transformer の幅（d_model）と層数。
   モデルが表現できる関数の複雑さを決める。train 正解率が上がるなら壁は容量起因だった、となる。
3. **学習率**（容量でも動かない場合: `LR=3e-4`）: 重みの更新式
