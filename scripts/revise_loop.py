...
採点モデル（pref-sentseq-section-triples / pref-bt-keep）の学習データが節・hunk 単位なので、これが正しい粒度である。

各反復で、節の現版から Cursor SDK で推敲案を複数生成し、
pref-sentseq-section-triples のスコアで順位付け、pref-bt-keep をゲート（元の節に対して
...
の self 基準の人間編集マージン分布（中央値 3.4、p25 1.8；節 keep valid、pref-sentseq-section-triples）を根拠に選ぶが、
...
def main() -> None:
  ...
  parser.add_argument("--min-margin", type=float, default=3.4, help="合格ライン（pref-sentseq-section-triples、元の節基準、self 基準分布の中央値）")
