...
- A1 学習側の三群: `stage_japanese_tech_writing.py`、`dok_a1_probe_gen.sh`、`check_a1_probe_smoke.py`（`make a1-probe-smoke-check` / `make a1-probe-check`）
...
- B 学習側の付け直し（329 件×3 対、比較できない可）: `build_b_rejudge_pairs.py`、`analyze_b_rejudge.py`（`make b-rejudge-judge`）
