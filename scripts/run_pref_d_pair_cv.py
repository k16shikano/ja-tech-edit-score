#!/usr/bin/env python3
"""D ペア BT/GPM の 5 分割 CV を実行し eval_cv_report.json を出す。"""
...
TRAIN_SCRIPT = ROOT / "scripts" / "train_pref_d_pair.py"
...
def load_oof_predictions(cv_dir: Path, *, mode: str) -> list[dict]:
  rows: list[dict] = []
  for fold in range(5):
...
def main() -> None:
  ...
  default_name = "pref-d-bt-modernbert" if args.mode == "bt" else "pref-d-gpm-modernbert"
  ...
  if report_path.is_file() and all(load_fold_summary(output_dir, fold) for fold in range(5)):
  ...
  oof_rows = load_oof_predictions(output_dir, mode=args.mode)
