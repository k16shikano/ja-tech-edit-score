#!/bin/bash
# DOK: 節三つ組みで setwise 文列型を human-top 損失で学ぶ
set -euo pipefail
cd /app
...
OUT="${OUT:-outputs/pref-setwise-section-human-top}"
...
echo "=== train pref-setwise human-top from section triples device=${DEVICE} ==="
...
mkdir -p "${art}/pref-setwise-section-human-top"
cp -a "${OUT}/." "${art}/pref-setwise-section-human-top/"
printf '%s\n' "unit=section" "teacher=section_triple_v1" "kind=pref-setwise-section-human-top" > "${art}/pref-setwise-section-human-top/DATA_UNIT.txt"
