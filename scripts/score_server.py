...
三軸で返す。
- 主軸（pref-sentseq-section-triples）: 教師データ B で学んだ文列型 Bradley-Terry。合否と分布ゲージに使う。
...
# 主軸は教師データ B の文列型 Bradley-Terry。ゲートは hunk keep の BT。
# 方向感度はアンカー学習版が別に担う。
DEFAULT_PRIMARY = ROOT / "outputs" / "pref-sentseq-section-triples"
...
DEFAULT_MIN_MARGIN = float(os.environ.get("MIN_MARGIN", "3.4"))
  ...
  result = {
    ...
    "notices": {
      "do_not_swap": (
        ...
        "主軸は変更があると正の評価値になりやすい傾向があるため、"
        "下書きと推敲を入れ替えて指定しないこと。"
      ),
      "no_storage": (
        "入力文章は評価計算のためにサーバへ送られる。"
        "サーバに長期間保存したり学習に使ったりはしない。"
      ),
