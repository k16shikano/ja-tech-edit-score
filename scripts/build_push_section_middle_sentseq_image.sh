#!/usr/bin/env bash
# 節ペア三つ組みの文列型学習イメージを build / push する（PLAN 工程 8-mid / DOK）。
...
TAG="${TAG:-pref-sentseq-section-triples:latest}"
IMAGE="${REGISTRY}/${TAG}"
echo "building and pushing ${IMAGE}"
docker buildx build --platform linux/amd64 \
  -f Dockerfile.section-middle-sentseq \
  -t "${IMAGE}" \
  --push \
  .
echo "pushed: ${IMAGE}"
echo "DOK: イメージ=${IMAGE}"
echo "  本番: 環境変数なし（既定 40 epoch / lr 3e-4）"
echo "  スモーク: EPOCHS=2"
