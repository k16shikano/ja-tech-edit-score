...
### 完了: Phase 3（`ead-gate-defect` / `ead-gate-level`）

#### G1（欠陥関門）

閾値: `epsilon` 0.0087、`char_ratio_low` 0.4914、`char_ratio_high` 2.1904、`draft_missing_high` 0.8179、`cand_novel_high` 0.8400。

| 集合 | n | 通過率 | 棄却内訳 |
|---|---|---|---|
| edit_sft_all_train_human | 1716 | 0.991 | addition 3, no_edit 4, omission 8 |
| d_all | 800 | 0.877 | no_edit 87, omission 11 |
| adapter_samples | 480 | 0.885 | no_edit 45, omission 10 |

D ラベル a の棄却は 11/267 = 0.041。acceptance は両項目 ✓。

**H2 に対して支持的。** 運用分布（adapter_samples）で 11.5% が棄却されており、関門は実際に発火している。ただし G1 は劣化検出器ではない。捕まえているのは no_edit と粗い欠落である。

#### G2（水準関門）

床の RPS に実装バグがあり（事例ごとの累積ではなく全体比率の累積を比較していた）、`common.py` の `rps_ordinal` で修正済み。修正後の値は次のとおり。
...
ラベル a 再現率は G2 0.691、床 0.636。4 値精度 0.465。$|\theta_k - \mathrm{mean}(g)| = 0.73$ で飽和なし。

学習曲線（draft 数 40 / 80 / 120 / 160）の valid RPS は 0.175 / 0.204 / 0.171 / 0.174、a 再現率は 0.800 / 0.855 / 0.709 / 0.673。

> #### ⚠ 再検討事項 R7: 入力特徴の全面禁止は過剰だった（対処済み・要再実行）
>
> D valid 上の単変量 AUC は次のとおりであった。
>
> | 特徴 | AUC |
> |---|---|
> | delta_chars | 0.489 |
> | char_ratio | 0.510 |
> | **edit_distance_norm** | **0.642** |
> | **punct_density_diff** | **0.602** |
>
> **Δ文字数の AUC は偶然水準を下回っている。** Δ文字数のみのモデルの 4 値精度も 0.400 で、多変量床 0.613 に遠く及ばない。すなわち **D のラベルは文字数で決まっていない**。
>
> これは重要な所見である。BRIEF.md の「文字数の違いだけを見て良し悪しを判断している傾向」は既存の `pref-*` 系列に現れた症状であって、**D のラベル自体は健全**であった。
>
> 一方、床 0.181 を支えているのは編集距離と句読点密度差である。本計画はこれらを交絡の疑いだけで一括禁止していたため、**有効な信号を捨てたまま G2 を学習していた**。
>
> **対処**: タスク 3-2 の入力特徴規定を特徴ごとの可否に改訂済み。編集距離と句読点密度差を加えて **G2 を再学習すること**。Δ文字数と文字数比は引き続き入れない。
>
> RPS が 0.18 前後まで下がれば原因は禁止の過剰であったと確定する。下がらなければ R8 に進む。

> #### ⚠ 再検討事項 R8: G2 は周辺分布予測器と同水準
>
> G2 の RPS 0.229 は周辺分布予測器の 0.228 と実質同じである。**$(x,y)$ から確率分布としての情報をほとんど学習していない。**
>
> a 再現率の差（0.691 対 0.636）も、valid のラベル a が 55 件であるから 38 件と 35 件の違いにすぎない。ここから何かを読むことはできない。
>
> 学習曲線は 40 drafts で平坦であり、**データ量は律速ではない**。元の指示にあった「未達なら D の追加アノテーションが最優先の投資先」という結論は**成り立たない**。この方向への投資を提案してはならない。
>
> R7 の再学習で改善しない場合、残る原因は**入力表現**である。凍結 $p_\theta$ の最終層隠れ状態の mean pooling は系列を潰すため、「どこがどう変わったか」が失われる。床が編集距離で AUC 0.642 を取っていることは、その情報が有効であることを示している。その場合は pooling の変更（差分位置の重み付け、トークン単位の対数尤度差の統計量など）を検討する。

> #### ⚠ 再検討事項 R9: 有限標本では較正が自動的に得られない
>
> `P(Y≥c)` の reliability は高確率側で大きく自信過剰であった。
>
> | bin | 予測 | 実測 |
> |---|---|---|
> | 0.8–1.0 | 0.956 | 0.681 |
...
> | 0.0–0.2 | 0.077 | 0.204 |
>
> 設計解説 §4.7 は「対数スコアが proper だから較正は自動的に得られる」と述べているが、これは漸近的な主張である。640 行・5-fold の規模では成り立たない。
>
> **帰結**: Phase 4 で `TAU_LEVEL` を確率の閾値として使う場合、**事後的な較正（温度スケーリング等）が要る**。較正は fold 内 valid で fit し、テスト側に漏らさないこと。

> #### ⚠ 再検討事項 R10: G1 の no_edit 棄却と G2 の水準判定が重複しうる
>
> D で no_edit 棄却が 87 件出ている。ラベル b は「下書きに相当」と定義されているため、no_edit 候補は定義上 b になる。すなわち G1 の no_edit 棄却は、G2 が担うはずの水準判定と重なっている。
>
> Phase 4 の合成で二重に効かないか確認すること。G1 で落ちた候補を G2 に通さない構成であれば問題ないが、棄却理由の内訳は記録すること。

> #### ⚠ 再検討事項 R11: H3 が未検証
>
> G2 は $s_\mathrm{den}$ を入力に含む。したがって a 再現率 0.691 が G2 の寄与なのか、$s_\mathrm{den}$ 単独で取れるものなのかが分からない。
>
> **未実施の ablation**:
>
> 1. $s_\mathrm{den}$ 単独で D valid のラベル a を予測したときの再現率
> 2. $s_\mathrm{den}$ を入力から抜いた G2 の a 再現率
>
> 1 が 0.691 に近ければ、**G2 は $s_\mathrm{den}$ の再表現でしかなく、H3 は偽**である。その場合、G2 を外して R に一本化する判断に実証的根拠がつく。
>
> R7 の再学習後に実施すること（入力特徴が変われば解釈も変わる）。
...
### 記録: 床の RPS の実装バグ（修正済み）

`floor.py` の旧 `rps` は、事例ごとの累積ではなく「valid 全体のラベル比率」と「予測確率の行平均」の累積を比較していた。結果として RPS 0.001 という、4 値精度 0.613 と両立しない値が出ていた。

**修正**: `scripts/ead/common.py` に `rps_ordinal`（K=4 なら 3 項の per-sample 累積 RPS）を置き、`floor.py` と `gate_level.py` から使用。

**以降の規約**: RPS を報告する際は、必ず**一様予測器**（K=4 で約 0.23）と**周辺分布予測器**を併記すること。これより大幅に低い値が出たら実装を疑う。
...
- **入力特徴の可否は特徴ごとに異なる（R7 により改訂）。**
  - **Δ文字数・文字数比は入れてはならない。** D valid 上の単変量 AUC が 0.489 / 0.510 と偶然水準であり、入れる利益がない一方、C での交絡リスクだけが残る
  - **編集距離（`edit_distance_norm`）と句読点密度差（`punct_density_diff`）は入れてよい。** AUC 0.642 / 0.602 と有効な信号であり、かつ文字数とは別の量である
  - いずれの場合も、Δ文字数は層化評価にのみ用いる
...
### Acceptance criteria (P3) — G1 判定済み / G2 再学習後に再判定

**G1: 充足**

- [x] A の人間推敲を 99% 以上通過させる（0.991）
- [x] D のラベル a を棄却する（11/267）

> ただし「棄却 > 0」という基準は弱すぎて情報にならない。G1 が劣化検出器でないことは結果から明らかであり、この基準は G1 の性能を測っていない。**運用分布での棄却率（adapter_samples で 11.5%）の方が H2 に対して情報量がある。**

**G2: 初回は未達。R7 の入力特徴を加えて再学習した上で再判定する**

- [ ] ラベル a 再現率が層内で床を上回る（初回 0.691 対 0.636。ただし 55 件中 3 件差で判定に足りない）
- [ ] RPS が床を下回る（初回 0.229 対 0.181。**周辺分布 0.228 と同水準**）
- [ ] reliability diagram が層別でも対角から大きく外れない（初回は高確率側で大きく自信過剰。R9）

> **元の指示「満たさない場合は D の追加アノテーションが最優先の投資先」は撤回する。** 学習曲線が 40 drafts で平坦であり、データ量は律速ではない（R8）。この方向への投資を提案してはならない。
>
> 再学習後も未達の場合は、入力表現の問題として R8 に従う。RPS の報告には必ず一様予測器と周辺分布予測器を併記すること。
...
| 4 前半 | P3: 関門 G1 / G2 | 3h | **G1 完了。G2 は R7 により再学習が要る** |
...
- **G2 は R7 の入力特徴を加えて再学習する。** その後 R11 の ablation を実施し、H3 の成否を判定する
- **P4 で `TAU_LEVEL` を確率閾値として使うなら、事後較正が要る**（R9）
...
- **全体値だけを報告し、層化を省略すること**
- **Δ文字数・文字数比をモデルの入力特徴に入れること。** 編集距離と句読点密度差は入れてよい（R7）
...
- **R7 の再学習で G2 の RPS が改善しない場合、入力表現の変更に進むか**（R8）
- **R11 の ablation の結果、H3 が偽と出た場合に G2 を外すか**
