...
**人間の編集が、オリジナル（下書き）に対して「意味は保持されているが、構成や表現が必ずよくなっているか」を見分けること。**

これに含まれないものは、今後の評価対象・成功指標にしない。

- 段落結合・過剰分割・段落順の機械的逆転（いわゆる `deg-*`）を正しく順位付けできるか
- 空行や段落境界だけの操作を罰せられるか

過去の v2（制御改悪）や構成負例まわりの `deg-*` 採点は探索用の記録であり、本線から外す。
...
v2 以降の方向（更新）: 節単位の素材は使ってよいが、評価候補は
**人間編集・下書き・意味を保った他推敲案**に限る。
機械的な段落結合・過剰分割・順序逆転は評価対象にしない。
...
節単位で「段落結合・過剰分割・順序逆転」を定義順位で測る試験を一度作ったが、
**今後の評価対象から外す**（本ドキュメント冒頭の目標を参照）。

理由の要約:

- 空行操作の正誤は、人間の編集目標（意味を保ちつつ構成・表現を良くする）と一致しない
- 選好モデルに空行ルールを教える／測ることは、浅い書式癖を誘発しやすい
- 本線は「下書きに対する人間編集（および意味を保った他推敲案）」の識別に戻す

スクリプト（`hard-eval-v2-build` 等）は履歴として残すが、新規採点・採否判定に使わない。
節単位の素材が必要な場合も、候補は **人間編集・下書き・他モデル推敲** に限り、`deg-*` は混ぜない。

## 三点比較（v2b / v2c）

節単位の下書き24件それぞれに、次の3候補を並べる試験。参考順位は「人間編集 > 機械推敲案 > コピー」。

- v2b: 人間編集 / **Fable** の推敲案 / 下書きのコピー（`data/hard_eval/bases_v2b_human_fable_copy.jsonl`）
- v2c: 人間編集 / **composer-2.5** の推敲案 / 下書きのコピー（`data/hard_eval/bases_v2c_human_machine_copy.jsonl`。`make v2c-composer-gen`（要 `CURSOR_API_KEY`）→ `make eval-v2c-composer` で構築・採点）

順位の成否だけでなく点差を見る。`scripts/analyze_hard_eval_margins.py`（`--mid fable` / `--mid machine`）で、
候補間の点差の分布と、機械案の相対位置（コピー=0、人間編集=1 に正規化した位置）を集計し、
`scripts/compare_hard_eval_margin_reports.py` で2モデルを並べる。
主に見る数値は「人間編集が機械案を上回った項目の割合（勝率）」と機械案の相対位置で、
これが「流暢な機械推敲案を人間の編集より高く採点する癖」の測定器になっている。
