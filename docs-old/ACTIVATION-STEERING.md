...
いま Qwen で見る問い：**推敲という軸が、LLM の表象で線型に分離できるか**（フェーズ A）。
対照ペアは **人手採用の節** で取り直す。hunk 系での第1回計測は履歴（下記）。
...
steering は対照対の**平均差分方向**という統計量だけを活性化に足す。
...
（第1回の数値は hunk 系ペアでの計測であり、この判定を確定させない。人手採用節での取り直し後に再適用する。）
...
**取り直し**では、人手採用の空行あり節（`data/edit_sft_section/` の draft/revised）を同じ形式で書き出してから抽出する。
`make steering-pairs` の入力差し替えは、取り直し時に行う。
...
## フェーズ A 第1回の結果（履歴・2026-07-15）

対照ペアは当時の `revision_pairs.jsonl`（dpo_curated 由来・約 6,055 件、hunk 系）だった。
**現行の人手採用節ペアではない。数値は取り直し前の参考であり、結論には使わない。**

全 6,055 ペア・14 書籍で DOK により抽出し、LOPO でプローブした（Qwen3-8B・mean-pool）。
...
- 単独分類はチャンス付近
- ペア差分は 0.64。書籍横断の方向は弱く見えるが、教師が hunk 系のため再確認が必要
- 対策として `reading` / `norms`＋最終トークン抽出を実装した（第2回用）

### 取り直しの方針（現行）

対照ペアを **人手採用の空行あり節**（`data/edit_sft_section/`）に差し替えてフェーズ A を測り直す。
手順（export → extract → probe）は上記「フェーズ A の実行」のまま使える。
`make steering-pairs` の入力を dpo_curated ではなく、採用済み draft/revised にする必要がある。
...
|------|------|
| 理論・計画（本ドキュメント） | あり |
| フェーズ A 第1回（mean-pool・hunk 系ペア） | 計測済み（履歴）。最良 micro 0.64。**現行結論には使わない** |
| フェーズ A 取り直し（人手採用節） | 未実施 |
| フェーズ A 第2回脚本（reading / norms） | あり・未計測 |
| フェーズ B / C | 未着手（取り直し後に判断） |
