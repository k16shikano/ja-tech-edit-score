...
**現行の対照ペア**: 取り直しでは人手採用の空行あり節（`data/edit_sft_section/` 由来）を使う。
下記手順の箱・抽出・プローブの流れはそのまま使える。
第1回（hunk 系 `revision_pairs`）の数値は [ACTIVATION-STEERING.md](ACTIVATION-STEERING.md) の履歴を参照（現行結論には使わない）。
